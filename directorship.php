...
//remove directorship
if(isset($_GET['rem'])){
	$stmt = $conn->prepare("DELETE FROM customer_director where id=?");
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	$sname =trim($_POST['sname']); $spe=trim($_POST['spe']);	$shareA=trim($_POST['shareA']);	$cname=trim($_POST['cname']);
		...
		if(checkempty($sname) && checkempty($spe) && checkempty($shareA)&& checkempty($cname)){
			...
			$stmt_in = $conn->prepare("SELECT sname, tin FROM customer_director where sname=? and tin=? and cname=?");
			$stmt_in->execute(array($sname,$app_id,$cname));
			$affected_rows_in = $stmt_in->rowCount();
			if($affected_rows_in >= 1) 
			{
				$stmt = $conn->prepare("UPDATE customer_director SET sname = ?, spe = ?, shareA = ?, cname = ?  where sname=? and tin=? and cname=? Limit 1");
				...
				$sth = $conn->prepare("INSERT INTO customer_director (sname, spe, shareA,cname,tin) VALUES (?,?,?,?,?)");
		...
		<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
			...
			<form role="form"  name="reg_form"  id="form" class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST">
				...
				<?php echo $notice_msg;?>
					...
					<div class="form-group">
						<label for="cname" class="control-label col-xs-5">Company Name <span style="color:red">*</span> : </label>
						<div class="col-xs-7">
					...
					<div class="form-group">
						<label for="spe" class="control-label col-xs-5">Company Phone / Email <span style="color:red">*</span> : </label>
						<div class="col-xs-7">
							<input type="text" class="form-control"  id="spe" name="spe" value="<?php echo $spe;?>" required="true" placeholder="Enter Company Phone Number Or Email" />
						</div>
					</div>
					<div class="form-group">
						<label for="shareA" class="control-label col-xs-5">Company Address <span style="color:red">*</span> : </label>
						<div class="col-xs-7">
					...
					<div class="form-group pull-right">
						<div class="col-xs-7">
							<input type="submit" name="proceed" style="margin-bottom:10px;padding:5px 20px 5px 20px" value="Add Directorship" class="btn btn-primary btn-md"></input>
						</div>
					</div>
				</form>
			</div>
