...
//remove shareholder
if(isset($_GET['rem'])){
	$stmt = $conn->prepare("DELETE FROM customer_auditor where id=?");
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	$sname =trim($_POST['sname']); $phone=trim($_POST['phone']);	$shareA=trim($_POST['shareA']);	$email=trim($_POST['email']);
		...
		if(checkempty($sname) && checkempty($phone) && checkempty($shareA)&& checkempty($email)){
			...
			$stmt_in = $conn->prepare("SELECT sname, phone, tin FROM customer_auditor where sname=? and tin=? and phone=?");
			$stmt_in->execute(array($sname,$app_id,$phone));
			$affected_rows_in = $stmt_in->rowCount();
			if($affected_rows_in >= 1) 
			{
				$stmt = $conn->prepare("UPDATE customer_auditor SET sname = ?, phone = ?, address = ?, email = ?  where sname=? and tin=? and phone=? Limit 1");
				...
				$sth = $conn->prepare("INSERT INTO customer_auditor (sname, phone, address,email,tin) VALUES (?,?,?,?,?)");
