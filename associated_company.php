...
if(isset($_GET['rem'])){
	$stmt = $conn->prepare("DELETE FROM customer_company where id=?");
...
{
	$sname =trim($_POST['sname']); $rc=trim($_POST['rc']);	$shareA=trim($_POST['shareA']);
		//check if email is valid
		if(checkempty($sname) && checkempty($rc) && checkempty($shareA)){
		
			$stmt_in = $conn->prepare("SELECT cname, tin FROM customer_company where cname=? and tin=? and rc=?");
			$stmt_in->execute(array($sname,$app_id,$rc));
			$affected_rows_in = $stmt_in->rowCount();
			if($affected_rows_in >= 1) 
			{
				$stmt = $conn->prepare("UPDATE customer_company SET cname = ?, rc = ?, address = ?  where cname=? and tin=? and rc=? Limit 1");
				if($stmt->execute(array($sname,$rc,$shareA,$sname,$app_id,$rc))){
				...
				$sth = $conn->prepare("INSERT INTO customer_company (cname, rc, address,tin) VALUES (?,?,?,?)");
				$sth->bindValue (1, $sname);
				$sth->bindValue (2, $rc);
				$sth->bindValue (3, $shareA);
				$sth->bindValue (4, $app_id);
				if($sth->execute()){
					$errPL="Success: The Associated Company Record Was Created Successfully - Verify!!!";
					$notice_msg='<div class="alert alert-success alert-dismissable">
				   <button type="button" class="close" data-dismiss="alert" 
					  aria-hidden="true">
					  &times;
			...
			<form role="form"  name="reg_form"  id="form" class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST">
				...
				<?php echo $notice_msg;?>
					<div class="form-group">
						<label for="sname" class="control-label col-xs-5">Company Name <span style="color:red">*</span> : </label>
						<div class="col-xs-7">
					...
					<div class="form-group">
						<label for="rc" class="control-label col-xs-5">RC Number <span style="color:red">*</span> : </label>
						<div class="col-xs-7">
							<input type="text" class="form-control"  id="rc" name="rc" value="<?php echo $rc;?>" required="true" placeholder="Enter Phone Number Or Email" />
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-xs-7">
							<textarea class="form-control"  rows="2" id="shareA" name="shareA" required="true" placeholder="Enter Company Contact Address">
								<?php echo $shareA;?>
							</textarea>
						</div>
					</div>
					<div class="form-group pull-right">
						<div class="col-xs-7">
							<input type="submit" name="proceed" style="margin-bottom:10px;padding:5px 20px 5px 20px" value="Add Associated Company" class="btn btn-primary btn-md"></input>
						</div>
					</div>
				</form>
			</div>
